...
Documento de referência técnica para desenvolvimento com React na plataforma Learning Engine. Baseado na documentação oficial (react.dev) e em padrões consolidados da comunidade.
...
function LessonPage() {
  const aiReview = useAIReview(lesson)
  const navigation = useLessonNavigation(course, lessonId)
...
```tsx
// Bom: composição
function LessonLayout({ children, footer }) {
  return (
    <div>
      <main>{children}</main>
...
const progress = useMemo(
  () => ({ total: totalLessons, completed: completedCount, percent: progressPercent }),
  [totalLessons, completedCount, progressPercent]
)
```
...
function reducer(state, action) {
  switch (action.type) {
    case 'complete':
      return { ...state, completed: [...state.completed, action.lessonId] }
    default:
      return state
  }
}
