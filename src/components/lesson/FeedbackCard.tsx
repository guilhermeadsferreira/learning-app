...
export function FeedbackCard({ type, message, xpGained, className }: FeedbackCardProps) {
    ...
    >
      <CardContent className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <p className={cn('font-medium', isSuccess ? 'text-emerald-400' : 'text-rose-400')}>
            {message}
          </p>
          {xpGained !== undefined && xpGained > 0 && (
            <p className="mt-1 text-sm text-emerald-400/80">+{xpGained} XP ganhos!</p>
          )}
        </div>
      </CardContent>
    </Card>
